...
function firstUpdateData(action) {
...
function updateHouseData(aname, action) {
    var areaId = $(".filter-area>li.active").attr("area-id");
    if (undefined == areaId) areaId = "";
    var startDate = $("#start-date").val();
    var endDate = $("#end-date").val();
...
$(document).ready(function () {
    ...
    $('#pick-area').click(function () {
        updateHouseData(getAreaNameFromHTML(), 'renew');
    })
    $('#search-button').click(function () {
        aname = getAreaNameFromHTML();
        let detail_position = $("#detail_position")[0].value;
        aname = aname + '-' + detail_position;
        updateHouseData(aname, 'renew');
    })

    // 在页面添加好城区选项信息后，更新展示房屋列表信息
    firstUpdateData("renew");
    // 获取页面显示窗口的高度
        ...
        if (c - b < windowHeight + 50) {
            ...
            if (!house_data_querying) {
                ...
                if (cur_page < total_page) {
                    ...
                    // 向后端发送请求，查询下一页房屋数据
                    updateHouseData(getAreaNameFromHTML());
                } else {
                    house_data_querying = false;
                }
            }
